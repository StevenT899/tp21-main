<template>
    <div class="map-container">
        <div class="filter-section flex flex-col md:flex-row gap-4 mb-4">
            ...
            <div class="filter-group">
                <label for="language-program" class="block text-sm font-medium text-gray-700 mb-1">Language
                    Program</label>
                <select id="language-program" v-model="filters.languageProgram"
                    class="w-full p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                    <option value="ALL">ALL</option>
                    <option v-for="language in availableLanguages" :key="language" :value="language">{{ language }}
                    </option>
                </select>
            </div>
        ...
        <div class="map-wrapper relative rounded-lg overflow-hidden border border-gray-300" style="height: 500px;">
            ...
            <div v-if="selectedSchool" class="school-popup absolute bg-white p-4 rounded-lg shadow-lg"
                ...
                <div class="flex justify-between items-start">
                    ...
                    <svg v-if="isInCompareList" xmlns="http://www.w3.org/2000/svg" width="20" height="20"
...
const initializeSchools = () => {
    ...
    map.loadImage(
        ...
        (error, image) => {
            ...
            map.addSource('school-polygons', {
                ...
                data: {
                    ...
                    features: schools.value.map(school => ({
                        ...
                        geometry: {
                            type: 'Polygon',
                            coordinates: JSON.parse(school.coordinates)
                        },
                        properties: { id: school.School_AGE_ID }
                    }))
                }
            })
            ...
            map.addLayer({
                id: 'school-polygons-fill',
                type: 'fill',
                source: 'school-polygons',
                paint: {
                    'fill-color': '#088',
                    'fill-opacity': 0.3
                }
...
onMounted(() => {
    fetch(`${import.meta.env.VITE_API_URL}/zoneSchools`)
        .then(response => response.json())
        .then(data => {
            schools.value = data;
            initializeMap();
